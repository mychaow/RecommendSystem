...
public class ReducerFour extends Reducer<Text, Text, Text, Text>{
...
//		super.setup(context);
		...
		hdfs = FileSystem.get(URI.create(srcavg), context.getConfiguration());
		Path avg_file = new Path(srcavg);
		Path sim_file = new Path(srcsim);
		avgin = hdfs.open(avg_file);
		FileStatus avgstat = hdfs.getFileStatus(avg_file); 
		avgbuffer = new byte[Integer.parseInt(String.valueOf(avgstat.getLen()))]; 
	    avgin.readFully(0, avgbuffer); 
	    simin = hdfs.open(sim_file);
	    FileStatus simstat = hdfs.getFileStatus(sim_file);
...
//			arg2.write(new Text(arg0 + "\t" + i), new Text("0"));
		}
		*/
